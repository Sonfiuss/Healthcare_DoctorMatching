...
class PreprocesingData:
    
    text_vectorize = TfidfVectorizer()
    X_tf_idf = text_vectorize.fit_transform(CleanData.Text["new_text"])
    ...
    text_tf_idf = pd.concat([CleanData.new_text["prompt"], df_tf_idf], axis = 1)
    text_tf_idf.to_csv(f"data_csv/tf_idf.csv", index=False)
    Text = CleanData.new_text
